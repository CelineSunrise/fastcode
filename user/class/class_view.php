<?php
  ...
  $queryB = "SELECT lecid from user_lectures where userid='".$userid."'";
  $Bresult = $mysqli->query($queryB) or die ("query error =>".$mysql->error);
  // $cr = $cresult->fetch_object();
  while($Br = $Bresult->fetch_object()){
    $Brarr[]=$Br->lecid;
  }
  if(!in_array($lecid, $Brarr)){
    echo "<script>
    alert('신청하지 않은 강좌입니다!');
    location.href = 'myclass.php';
  </script>";
  }
